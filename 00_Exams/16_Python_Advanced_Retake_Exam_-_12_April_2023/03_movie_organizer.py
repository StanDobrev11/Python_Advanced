...
place. Mary needs your help to organize her movies.
Write a function called "movie_organizer" that groups movies by genre. The function will receive a different
number of arguments, passed as tuples containing two elements: the first one is the movie's name, and the
second is the genre for example ("Movie Name", "Genre").
The function should sort the movies by their genre. Arrange Mary's collection by the number of movies in each
genre in descending order. If two or more genres have the same number of movies, return them in ascending order
...
def movie_organizer(*args):
    ...
    for movie, gender in args:
        if gender not in catalogue:
            catalogue[gender] = set()
        catalogue[gender].add(movie)

    output_data = ''
    for key, values in sorted(catalogue.items(), key=lambda x: (-len(x[1]), x[0])):
        output_data += f"{key} - {len(values)}\n"
